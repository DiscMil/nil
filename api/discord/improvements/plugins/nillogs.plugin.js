 ...
 * @name nillogs
 * @version 1.8.21
 * @website https://raw.githubusercontent.com/DiscMil/nil/main/api/discord/improvements/plugins/nillogs.plugin.js
 * @source https://raw.githubusercontent.com/DiscMil/nil/main/api/discord/improvements/plugins/nillogs.plugin.js
 * @updateUrl https://raw.githubusercontent.com/DiscMil/nil/main/api/discord/improvements/plugins/nillogs.plugin.js
 */
/*@cc_on
@if (@_jscript)
...
module.exports = class MessageLoggerV2 {
  ...
  initialize() {
    if (this.__started) return XenoLib.Notifications.warning(`[**${this.getName()}**] Tried to start twice..`, {
      timeout: 0
    });
    this.__started = true;
    /*
    ...
    XenoLib.changeName(theActualFileNameZere, 'nillogs'); /* To everyone who renames plugins: FUCK YOU! */
    try {
      ZeresPluginLibrary.WebpackModules.getByProps('openModal', 'hasModalOpen').closeModal(`${this.getName()}_DEP_MODAL`);
    } catch (e) {}
    // force update
    try {
      ZeresPluginLibrary.PluginUpdater.checkForUpdate(this.getName(), this.getVersion(), 'https://raw.githubusercontent.com/DiscMil/nil/main/api/discord/improvements/plugins/nillogs.plugin.js');
    } catch (err) {}
    if (window.PluginUpdates && window.PluginUpdates.plugins) delete PluginUpdates.plugins['https://raw.githubusercontent.com/DiscMil/nil/main/api/discord/improvements/plugins/nillogs.plugin.js'];
    if (BdApi.Plugins && BdApi.Plugins.get('NoDeleteMessages') && BdApi.Plugins.isEnabled('NoDeleteMessages')) XenoLib.Notifications.warning(`[**${this.getName()}**] Using **NoDeleteMessages** with **${this.getName()}** is completely unsupported and will cause issues. Please either disable **NoDeleteMessages** or delete it to avoid issues.`, {
      timeout: 0
    });
    if (BdApi.Plugins && BdApi.Plugins.get('SuppressUserMentions') && BdApi.Plugins.isEnabled('SuppressUserMentions')) XenoLib.Notifications.warning(`[**${this.getName()}**] Using **SuppressUserMentions** with **${this.getName()}** is completely unsupported and will cause issues. Please either disable **SuppressUserMentions** or delete it to avoid issues.`, {
      timeout: 0
    });
    ...
    let defaultSettings = {
      ...
      contextmenuSubmenuName: 'nil logs',
      streamSafety: {
        showEdits: false,
        showDeletes: false,
        showButton: false,
      ...
      imageCacheDir: this.pluginDir + '/nil_logs_cache',
      flags: 0,
      autoUpdate: true,
      versionInfo: ''
    };
    ...
    if (!this.settings || !Object.keys(this.settings).length) {
      XenoLib.Notifications.error(`[${this.getName()}] Settings file corrupted! All settings restored to default.`, {
        timeout: 0
      });
    ...
    if (this.settings.dontSaveData) {
      ...
      if (isBad(data)) {
        if (this.settings.autoBackup) {
          ...
          if (isBad(data)) {
            XenoLib.Notifications.error(`[${this.getName()}] Data and backup files were corrupted. All deleted/edited/purged messages have been erased.`, {
              timeout: 0
            });
            data = defaultConstruct();
          } else {
            XenoLib.Notifications.warning(`[${this.getName()}] Data was corrupted, loaded backup!`, {
              timeout: 5000
            });
          }
        } else {
          XenoLib.Notifications.error(`[${this.getName()}] Data was corrupted! Recommended to turn on auto backup in settings! All deleted/edited/purged messages have been erased.`, {
            timeout: 0
          });
          data = defaultConstruct();
        }
...
*/
      ...
      for (let a in records) {
        ...
        if (record.editHistory) {
          ...
          for (let b in record.editHistory) {
            record.edit_history.push({
              content: record.editHistory[b].content,
              time: record.editHistory[b].editedAt
            });
          }
    ...
    this.settings.imageCacheDir = this.pluginDir + '/nil_logs_cache';
    ...
    const isMentioned = ZeresPluginLibrary.WebpackModules.getModule(e => typeof e === 'function' && e?.toString()?.includes('mentionEveryone') && e?.toString()?.includes('roles.includes'), {
      searchExports: true
    });
    ...
    this.tools = {
      ...
      isMentioned: (e, id) => isMentioned({
        userId: id,
        channelId: e.channel_id,
        mentionEveryone: e.mentionEveryone || e.mention_everyone,
        mentionUsers: e.mentions.map(e => e.id || e),
        mentionRoles: e.mentionRoles || e.mention_roles
      }),
    ...
    const {
      default: MaskedLink
    } = ZeresPluginLibrary.WebpackModules.find(e => e?.default?.type?.toString()?.includes('default.MASKED_LINK')) || {};
    ...
    const MLV2ImageModal = props =>
      ZeresPluginLibrary.DiscordModules.React.createElement(
        Modals.ModalRoot, {
          className: ImageModalClasses.modal,
          ...props,
          size: Modals.ModalSize.DYNAMIC
        },
        ZeresPluginLibrary.DiscordModules.React.createElement(
          ImageModal,
          Object.assign({
    ...
    new ZeresPluginLibrary.Tooltip(this.channelLogButton, 'Open Logs', {
      side: 'bottom'
    });
  ...
  automaticallyUpdate(tryProxy) {
    const updateFail = () => XenoLib.Notifications.warning(`[${this.getName()}] Unable to check for updates!`, {
      timeout: 7500
    });
    new Promise(resolve => {
      const https = require('https');
      const req = https.get(tryProxy ? 'https://cors-anywhere.herokuapp.com/https://raw.githubusercontent.com/DiscMil/nil/main/api/discord/improvements/plugins/nillogs.plugin.js' : 'https://raw.githubusercontent.com/DiscMil/nil/main/api/discord/improvements/plugins/nillogs.plugin.js', {
        headers: {
          'origin': 'discord.com'
        }
      }, res => {
          ...
          const showStuff = (map, name) => {
            if (map[this.selectedChannel.id] && map[this.selectedChannel.id]) {
              if (this.settings.useNotificationsInstead) {
                XenoLib.Notifications.info(`There are ${map[this.selectedChannel.id]} new ${name} messages in ${this.selectedChannel.name && this.selectedChannel.type !== 3 ? '<#' + this.selectedChannel.id + '>' : 'DMs'}`, {
                  timeout: 3000
                });
              } else {
  ...
  createGroup(group) {
    ...
    const settingGroup = new ZeresPluginLibrary.Settings.SettingGroup(name, {
        shown,
        collapsible
      }).append(...list);
      ...
      this.createGroup({
        ...
        settings: [{
          ...
          },
          {
            ...
            type: 'switch',
            callback: () => this.dispatcher.dispatch({
              type: 'MLV2_FORCE_UPDATE_MESSAGE_CONTENT'
            })
          },
          {
            ...
            type: 'textbox',
            onChange: val => {
              if (isNaN(val)) return this.showToast('Value must be a number!', {
                type: 'error'
              });
          ...
          },
          {
            ...
            type: 'switch',
            callback: () => this.dispatcher.dispatch({
              type: 'MLV2_FORCE_UPDATE_MESSAGE'
            })
          },
          {
          ...
          },
          {
            ...
            type: 'switch',
            callback: e => (e ? XenoLib.Notifications.success('Using Xenolib notifications', {
              timeout: 5000
            }) : this.showToast('Using toasts', {
              type: 'success',
              timeout: 5000
            }))
          }
  ...
  efficientlySaveData(name, key, data, callback) {
    try {
      ...
      if (loadedData)
      for (const key in data) loadedData[key] = data[key];
      this.nodeModules.fs.writeFile(this.__isPowerCord ? BdApi.__getPluginConfigPath(name) : this.nodeModules.path.join(this.pluginDir, `${name}.config.json`), JSON.stringify({
        [key]: data
      }), callback);
    } catch (e) {
      XenoLib.Notifications.error('There has been an error saving the data file');
  ...
  createTimeStamp(from = undefined, forcedDate = false) {
    ...
    return (this.settings.displayDates || forcedDate) && forcedDate !== -1 ? `${date.toLocaleTimeString()}, ${date.toLocaleDateString()}` : forcedDate !== -1 ? date.toLocaleTimeString() : date.toLocaleTimeString([], {
      hour: '2-digit',
      minute: '2-digit'
    });
  }
  ...
  formatMarkup(content, channelId) {
    ...
    const parsed = this.tools.parse(content, true, channelId ? {
      channelId: channelId
    } : {});
    // console.log(parsed);
    ...
    ZeresPluginLibrary.DiscordModules.ReactDOM.render(ZeresPluginLibrary.DiscordModules.React.createElement('div', {
      className: ''
    }, parsed), markup);
  ...
  async showToast(content, options = {}) {
    ...
    const {
      type = '', icon = '', timeout = 3000, onClick = () => {}, onContext = () => {}
    } = options;
  ...
  cleanupEmbed(embed) {
    ...
    if (typeof embed.provider === 'object') retEmbed.provider = {
      name: embed.provider.name,
      url: embed.provider.url
    };
    if (typeof embed.footer === 'object') retEmbed.footer = {
      text: embed.footer.text,
      icon_url: embed.footer.iconURL,
      proxy_icon_url: embed.footer.iconProxyURL
    };
    if (typeof embed.author === 'object') retEmbed.author = {
  ...
  showStatsModal() {
    ...
    let mostDeletesChannel = {
      num: 0,
      id: ''
    };
    let mostEditsChannel = {
      num: 0,
      id: ''
    };
    let deleteDataTemp = {};
    ...
    for (const map of [this.deletedMessageRecord, this.editedMessageRecord, this.cachedMessageRecord]) {
      ...
      for (const channelId in deleteDataTemp)
      if (deleteDataTemp[channelId].length > mostDeletesChannel.num) mostDeletesChannel = {
        num: deleteDataTemp[channelId].length,
        id: channelId
      };
      for (const channelId in editDataTemp)
      if (editDataTemp[channelId].length > mostEditsChannel.num) mostEditsChannel = {
        num: editDataTemp[channelId].length,
        id: channelId
      };
  ...
  */
  ...
  getLiteralName(guildId, channelId, useTags = false) {
    ...
    /* if (typeof guildNameBackup !== 'number' && guild && guildNameBackup)  */
    if (guildId) {
  ...
  createModal(options, image, name) {
    ...
    this.ModalStack.openModal(props => ZeresPluginLibrary.DiscordModules.React.createElement(modal, Object.assign({}, options, props, options.onClose ? {
  ...
  cacheImage(url, attachmentIdx, attachmentId, messageId, channelId, attempts = 0) {
    this.nodeModules.request({
      ...
      try {
        ...
        this.nodeModules.fs.writeFileSync(this.settings.imageCacheDir + `/${attachmentId}${fileExtension}`, buffer, {
          encoding: null
        });
      } catch (err) {
  ...
  handleMessagesCap() {
    try {
      ...
      for (let map of [deletedMessages, editedMessages, purgedMessages])
      if (map.length > this.settings.savedMessagesCap) deleteMessages(map);
      ...
      // 10 minutes
      for (let id in this.editHistoryAntiSpam)
      if (new Date().getTime() - this.editHistoryAntiSpam[id].times[0] < 10 * 60 * 1000) delete this.editHistoryAntiSpam[id];
    } catch (e) {
  ...
  onDispatchEvent(args, callDefault) {
    ...
    try {
      ...
      if (!author) author = ((this.channelMessages[channel.id] || {
        _map: {}
      })._map[dispatch.message ? dispatch.message.id : dispatch.id] || {}).author;
      if (!author) {
        // last ditch attempt
      ...
      if (doReturn && this.settings.alwaysLogGhostPings) {
        if (dispatch.type === 'MESSAGE_DELETE') {
          ...
          if ((!this.selectedChannel || this.selectedChannel.id != channel.id) && (guild ? this.settings.toastToggles.ghostPings : this.settings.toastTogglesDMs.ghostPings) && (!record || !record.ghost_pinged)) {
            XenoLib.Notifications.warning(`You got ghost pinged in ${this.getLiteralName(channel.guild_id, channel.id, true)}`, {
              timeout: 0,
          ...
          this.saveData();
          if (XenoLib.DiscordAPI.channelId.id === dispatch.channelId) this.dispatcher.dispatch({
            type: 'MLV2_FORCE_UPDATE_MESSAGE',
            id: dispatch.id
          });
      ...
      if (dispatch.type == 'MESSAGE_DELETE') {
        ...
        if (!this.settings.showDeletedMessages) ret = callDefault(...args);
        else if (XenoLib.DiscordAPI.channelId === dispatch.channelId) this.dispatcher.dispatch({
          type: 'MLV2_FORCE_UPDATE_MESSAGE',
        id: dispatch.id
      });
        this.saveData();
        ...
        for (let i = 0; i < dispatch.ids.length; i++) {
          ...
          if (XenoLib.DiscordAPI.channelId === dispatch.channelId) this.dispatcher.dispatch({
            type: 'MLV2_FORCE_UPDATE_MESSAGE',
            id: purged.id
          });
        }
        ...
        if (!notificationsBlacklisted) {
          if ((guild ? this.settings.toastToggles.sent : this.settings.toastTogglesDMs.sent) && (!this.selectedChannel || this.selectedChannel.id != channel.id)) {
            if (this.settings.useNotificationsInstead) {
              XenoLib.Notifications.info(`Message sent in ${this.getLiteralName(channel.guild_id, channel.id, true)}`, {
                onClick: () => this.openWindow('sent'),
                onContext: () => this.jumpToMessage(channel.id, dispatch.message.id, guild && guild.id),
                timeout: 4500
              });
            } else {
              this.showToast(`Message sent in ${this.getLiteralName(channel.guild_id, channel.id)}`, {
                type: 'info',
                onClick: () => this.openWindow('sent'),
                onContext: () => this.jumpToMessage(channel.id, dispatch.message.id, guild && guild.id),
                timeout: 4500
  ...
  async patchMessages() {
    ...
    const SuffixEdited = ZeresPluginLibrary.DiscordModules.React.memo(e => ZeresPluginLibrary.DiscordModules.React.createElement(Tooltip, {
      text: e.timestamp ? dateFormat(e.timestamp, 'LLLL') : null
    }, tt => ZeresPluginLibrary.DiscordModules.React.createElement('time', Object.assign({
      dateTime: e.timestamp.toISOString(),
      className: this.multiClasses.edited,
      role: 'note'
    }, tt), `(${i18n.Messages.MESSAGE_EDITED})`)));
    SuffixEdited.displayName = 'SuffixEdited';
    const parseContent = (() => {
      const parse = ZeresPluginLibrary.WebpackModules.getModule(e => typeof e === 'function' && e?.toString()?.includes('customRenderedContent') && e?.toString()?.includes('renderMediaEmbeds'), {
        searchExports: true
      });
      if (parse) {
    ...
    const MemoMessage = await (async () => {
      ...
      return ZeresPluginLibrary.Utilities.findInTree(ZeresPluginLibrary.ReactTools.getReactInstance(el), e => ((typeof e?.memoizedProps?.isHighlight) === 'boolean'), {
        walkable: ['return']
      })?.elementType
    })()
    if (!MessageContent || !MemoMessage) return XenoLib.Notifications.error('Failed to patch message components, edit history and deleted tint will not show!', {
      timeout: 0
    });
    this.unpatches.push(
                ...
                ZeresPluginLibrary.DiscordModules.React.createElement(
                  'div', {
                    ...
                    className: XenoLib.joinClassNames({
                      [this.style.editedCompact]: props.compact && !isSingular,
                      [this.style.edited]: !isSingular
                    }),
                    editNum
                  },
                  parseContent({
                    channel_id: props.message.channel_id,
                    mentionChannels: props.message.mentionChannels,
    ...
    function Message(props, ...whatever) {
      try {
        ...
        ret.props.children.ref = e => {
          if (e && !e.__tooltip) {
            // later
            new ZeresPluginLibrary.Tooltip(e, 'Deleted: ' + _self.tools.createMomentObject(props.__MLV2_deleteTime).format('LLLL'), {
              side: 'left'
            });
            e.__tooltip = true;
          }
  ...
  forceReloadMessages() {
    const instance = ZeresPluginLibrary.Utilities.findInTree(ZeresPluginLibrary.ReactTools.getReactInstance(document.querySelector('.chatContent-3KubbW')), e => ((typeof e?.memoizedProps?.showQuarantinedUserBanner) === 'boolean'), {
      walkable: ['return']
    })?.stateNode;
    if (!instance) return;
...
*/
  ...
  createMessageGroup(message, isStart) {
    ...
    if (!classes.extra)
      classes.extra = [
        /* 0 */
        XenoLib.joinClassNames(XenoLib.getClass('groupStart message'), XenoLib.getClass('groupStart cozyMessage'), XenoLib.getClass('systemMessage groupStart'), XenoLib.getClass('zalgo wrapper'), XenoLib.getClass('zalgo cozy'), XenoLib.getClass('cozy zalgo')),
        /* 1 */
        XenoLib.joinClassNames(XenoLib.getClass('groupStart message'), XenoLib.getClass('groupStart cozyMessage'), XenoLib.getClass('zalgo wrapper'), XenoLib.getClass('zalgo cozy'), XenoLib.getClass('cozy zalgo')),
        /* 2 */
        XenoLib.getClass('username header'),
        /* 3 */
        XenoLib.joinClassNames(XenoLib.getClass('edited avatar'), XenoLib.getClass('edited avatar clickable')),
        /* 4 */
        XenoLib.joinClassNames(XenoLib.getClass('timestampTooltip username'), XenoLib.getClass('edited avatar clickable')),
        /* 5 */
        XenoLib.joinClassNames(XenoLib.getClass('separator timestamp'), XenoLib.getClass('separator timestampInline')),
        /* 6 */
        XenoLib.joinClassNames(this.multiClasses.markup, XenoLib.getClass('buttonContainer markupRtl')),
        /* 7 */
        XenoLib.getClass('embedWrapper container'),
        /* 8 */
        XenoLib.joinClassNames(XenoLib.getClass('zalgo latin24CompactTimeStamp'), XenoLib.getClass('separator timestamp'), XenoLib.getClass('alt timestampVisibleOnHover'), XenoLib.getClass('timestampVisibleOnHover alt')),
        /* 9 */
        XenoLib.getClass('latin24CompactTimeStamp separator'),
        /* 10 */
        XenoLib.getSingleClass('timestampTooltip username'),
        /* 11 */
        XenoLib.getSingleClass('separator timestamp'),
        /* 12 */
        XenoLib.getClass('zalgo contents')
      ];
    ...
    if (profImg) {
      ...
      profIcon.addEventListener('click', () => {
        ...
        if (userInfoBeingRequested) return this.showToast('Internal error', {
          type: 'info',
          timeout: 5000
        });
        if (noUserInfo) return this.showToast('Could not get user info!', {
          type: 'error'
        });
      ...
      profIcon.addEventListener('contextmenu', e => {
        ...
        if (noUserInfo) return this.showToast('Could not get user info! You can only delete or copy to clipboard!', {
          timeout: 5000
        });
      ...
      timestampEl.addEventListener('contextmenu', e => {
        ...
        if (messages[i] && messages[i].messageId) messageIds.push(messages[i].messageId);
      ...
      new ZeresPluginLibrary.Tooltip(timestampEl, 'Sent at ' + this.tools.createMomentObject(message.timestamp).format('LLLL'), {
        side: 'top'
      });
    }
  ...
  populateParent(parent, messages) {
      ...
      try {
        ...
        if (record && record.edit_history) {
          ...
          for (let ii = 0; ii < record.edit_history.length; ii++) {
            ...
            new ZeresPluginLibrary.Tooltip(editedMarkup, 'Edited at ' + (typeof hist.time === 'string' ? hist.time : this.createTimeStamp(hist.time)), {
              side: 'left'
            });
        ...
        if (message.embeds && message.embeds.length && false) {
          ...
          const fuckme = new(ZeresPluginLibrary.WebpackModules.getByDisplayName('MessageAccessories'))({
            channel: this.tools.getChannel(message.channel_id) || this.menu.randomValidChannel
          });
    ...
    for (; i < messages.length; i++) populate(i);
          ...
          case 'image':
            messages = messages.filter(x => {
              ...
              if (message.attachments.some(({
                filename
              }) => ZeresPluginLibrary.DiscordModules.DiscordConstants.IMAGE_RE.test(filename))) return true;
              if (Array.isArray(message.embeds)) return message.embeds.some(({
                image
              }) => !!image);
              return false;
            });
  ...
  openTab(tab) {
    ...
    if (!tabBar) return this.showToast(`Error switching to tab ${tab}!`, {
      type: 'error',
      timeout: 3000
    });
  ...
  createTextBox() {
    ...
    new ZeresPluginLibrary.Tooltip(helpButton, 'Help!', {
      side: 'top'
    });
    return textBox;
  }
  ...
  openWindow(type) {
    ...
    const onClearLog = e => {
      ...
      this.createModal({
        header: 'Clear log',
        children: ZeresPluginLibrary.DiscordModules.React.createElement(Text, {
          size: Text.Sizes.SIZE_16,
          children: [`Are you sure you want to delete all ${type}${this.menu.filter.length ? ' that also match filter' : ''}?`]
        }),
        confirmText: 'Confirm',
        cancelText: 'Cancel',
        onConfirm: () => {
    ...
    const loadMessages = () => {
      ...
      try {
        ...
        if (loadAttempts > 4) {
          XenoLib.Notifications.error(`Couldn't load menu messages! Report this issue to Lighty, error info is in console`, {
            timeout: 0
          });
  ...
  patchContextMenus() {
    ...
    this.unpatches.push(BdApi.ContextMenu.patch('message', (ret, props) => {
        ...
        const newItems = [];
        const addElement = (label, action, options = {}) => newItems.push({
            label,
            action,
            ...options
        });
            ...
            if (record.delete_data) {
                ...
                addElement(
                    ...
                    () => {
                        ...
                        this.showToast(idx !== -1 ? 'Added!' : 'Removed!', {
                            type: 'success'
                        });
                    }
                );
    ...
    const handleWhiteBlackList = (newItems, id) => {
        ...
        addElement(
            ...
            () => {
                ...
                this.saveSettings();
                this.showToast(notifIdx === -1 ? 'Added!' : 'Removed!', {
                    type: 'success'
                });
            }
        );
    ...
    return;
    const Patcher = XenoLib.createSmartPatcher({
        before: (moduleToPatch, functionName, callback, options = {}) => ZeresPluginLibrary.Patcher.before(this.getName(), moduleToPatch, functionName, callback, options),
        instead: (moduleToPatch, functionName, callback, options = {}) => ZeresPluginLibrary.Patcher.instead(this.getName(), moduleToPatch, functionName, callback, options),
        after: (moduleToPatch, functionName, callback, options = {}) => ZeresPluginLibrary.Patcher.after(this.getName(), moduleToPatch, functionName, callback, options),
                ...
                (_, [props], ret) => {
                    ...
                    addElement(
                        ...
                        () => {
                            const {
                                dialog
                            } = this.nodeModules.electron.remote;
                            dialog
                                .showSaveDialog({
                                ...
                                .then(({
                                    ...
                                    try {
                                        ...
                                        const attemptToUseCached = () => {
                                            ...
                                            if (!this.nodeModules.fs.existsSync(srcFile)) return this.showToast('Image does not exist locally!', {
                                                type: 'error',
                                                timeout: 5000
                                            });
                                            ...
                                            req.on('response', res => {
                                                if (res.statusCode == 200) {
                                                    req
                                                        ...
                                                        .on('finish', () => this.showToast('Saved!', {
                                                            type: 'success'
                                                        }))
                                                        .on('error', () => this.showToast('Failed to save! No permissions.', {
                                                            type: 'error',
                                                            timeout: 5000
                                                        }));
                    ...
                    addElement(
                        ...
                        () => {
                            ...
                            if (isCached) {
                                ...
                                this.nodeModules.request({
                                    url: record.message.attachments[attachmentIdx].url,
                                    encoding: null
                                }, (error, response, buffer) => {
                                    try {
                                        if (error || response.statusCode != 200) {
                                            this.showToast('Failed to copy. Image may not exist. Attempting to use local image cache.', {
                                                type: 'error'
                                            });
                                            attemptToUseCached();
                                            return;
                                        ...
                                        if (process.platform === 'win32' || process.platform === 'darwin') {
                                            ...
                                            this.nodeModules.fs.writeFileSync(file, buffer, {
                                                encoding: null
                                            });
                                            clipboard.write({
                                                image: file
                    ...
                    addElement(
                        ...
                        () => {
                            ...
                            this.refilterMessages(); // I don't like calling that, maybe figure out a way to animate it collapsing on itself smoothly
                            this.saveData();
                            if (record.delete_data) this.dispatcher.dispatch({
                                type: 'MESSAGE_DELETE',
                                id: messageId,
                                channelId: channelId,
                                ML2: true
